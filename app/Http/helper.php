    ...
    function cek_stok_warning ($min){
        ...
        if (count($minim) > 0) {
            for ($i=0;$i<sizeof($minim);$i++) {
                 $warning[$minim[$i]]="Stok Obat ". get_value('obat',$minim[$i],'nama') . " " . get_value('obat',$minim[$i],'jenis'). " " . get_value('obat',$minim[$i],'dosis') . " " . get_value('obat',$minim[$i],'satuan') ." Mulai Menipis atau Sudah Habis";    
            return $warning;                     
            }
        }
        $warning = array();
    ...
    function validasi_stok($resultanresep){
        ...
        if ($habis !== NULL) {
            if (is_array($habis)) {
                $i=0;
                foreach ($habis as $h) {               
                    $errors['resep'[$i]] = 'Stok Obat '. get_value('obat',$h,'nama') . ' ' . get_value('obat',$h,'jenis'). ' ' . get_value('obat',$h,'dosis') . ' ' . get_value('obat',$h,'satuan') .' Tidak Cukup!';
                    $i++;
            }
                if (isset($errors)) {
                return $errors;
                ...
                $errors['resep'] = 'Stok Obat '. get_value('obat',$habis,'nama') . ' ' . get_value('obat',$habis,'jenis'). ' ' . get_value('obat',$habis,'dosis') . ' ' . get_value('obat',$habis,'satuan') .' Tidak Cukup!';
                if (isset($errors)) {
                return $errors;
                }
                else {return NULL;}
